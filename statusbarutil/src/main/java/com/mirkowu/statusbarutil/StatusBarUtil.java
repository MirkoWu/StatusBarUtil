package com.mirkowu.statusbarutil;
...
public class StatusBarUtil {
    ...
    /**
     * 设置状态栏全透明 已经留出statusBar高度
     * <p>
     * 但是底色可能还需要调整           {@link #setStatusBarColor(Activity, int, int)}
     * 如果还要修改字体（黑 或 白）      {@link #setStatusBarLightMode(Activity)}
     *
    ...
    public static void setTransparent(Activity activity) {
        ...
        setImmersiveTransparentStatusBar(activity);
    ...
    /**
     * 设置根布局参数 setFitsSystemWindows=true 根部局顶部增加一个statusBar登高的padding
     */
    ...
    /**
     * 针对根布局是 CoordinatorLayout, 使状态栏完全透明
     *
     * @param activity
     */
    public static void setTransparentForCoordinatorLayout(Activity activity) {
        //设置沉浸式透明状态栏
        setImmersiveTransparentStatusBar(activity);
    }
     ...
     * @param color    状态栏颜色值
     */
    public static void setStatusBarColorForSwipeBack(Activity activity, int color) {
        setStatusBarColorForSwipeBack(activity, color, DEFAULT_STATUS_BAR_ALPHA);
    }
    ...
    /**
     * 设置亮色模式 带有不支持亮色模式时的处理方式
     *
     * @param activity
     * @param isUseSystemStatusBarWhenNotSupport 当不支持亮色模式时 是否使用系统的StatusBar
     * @return true not support
     */
    public static boolean setStatusBarLightModeWithNoSupport(Activity activity, boolean isUseSystemStatusBarWhenNotSupport) {
        if (setStatusBarLightMode(activity) == 0) {
            if (isUseSystemStatusBarWhenNotSupport) {
                setFitsSystemWindows(activity);
     ...
     *
     * @param activity
     * @param statusBarColorIdWhenNoSupport 当不支持亮色模式时 设置statusBar的颜色
     */
    public static boolean setStatusBarLightModeWithNoSupport(Activity activity, @ColorInt int statusBarColorIdWhenNoSupport) {
        if (setStatusBarLightMode(activity) == 0) {
            setFitsSystemWindows(activity);
            setStatusBarColor(activity, statusBarColorIdWhenNoSupport);
            return true;
        }
        return false;
    }
     ...
     *
     * @param activity
     * @param useColorIdWhenNoSupport 当不支持亮色模式时  设置statusBar的颜色
     * @param statusBarAlpha          statusBar的透明度
     */
    public static boolean setStatusBarLightModeWithNoSupport(Activity activity, @ColorInt int useColorIdWhenNoSupport,
                                                             @IntRange(from = 0, to = 255) int statusBarAlpha) {
        if (setStatusBarLightMode(activity) == 0) {
            setFitsSystemWindows(activity);
            setStatusBarColor(activity, useColorIdWhenNoSupport, statusBarAlpha);
            return true;
        }
        return false;
    }
    ...
    public static int setStatusBarDarkMode(Activity activity) {
        int result = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (setMIUISetStatusBarLightMode(activity, false)) {
                result = 1;
